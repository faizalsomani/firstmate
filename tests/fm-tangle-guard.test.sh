...
# A fake tmux that reports FM_FAKE_PANE_PATH as the post-`cd` pane cwd (so the
# spawn's worktree-confirmation poll resolves to a path we control), names the
# session on '#S', and swallows window ops. Echoes the fakebin dir. The fake
# treehouse leases that same FM_FAKE_PANE_PATH so the leased path and the pane's
# reported cwd agree, exactly as a real lease + successful pane cd would.
make_spawn_fakebin() {
  local dir=$1 fakebin
...
#     are disabled so the fm-<id> name survives the leased worktree's cd;
#   - the leased-worktree cd send-keys and the worktree-confirmation wait loop
#     target that stable window id, never the (possibly-renamed) name - a lost
#     name would let display-message fall back to the active client's window and
#     misread firstmate's OWN pane as the worktree, tangling a hook into the
#     primary checkout.
...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): the leased-worktree cd and the worktree wait loop target the
  # stable id. The worktree is durably leased from firstmate's own process
  # first (treehouse get --lease --lease-holder <task-id>), then the pane is
  # told to cd there.
  assert_grep "get --lease --lease-holder rec-win-gg7" "$rec" \
    "the worktree must be durably leased under the task id before the pane is touched"
  assert_grep "send-keys -t @spawnwid cd '$wt' Enter" "$rec" \
    "the leased worktree cd must be sent to the stable window id"
  assert_grep "display-message -p -t @spawnwid #{pane_current_path}" "$rec" \
    "the worktree confirmation wait loop must query the stable window id, not the name"
