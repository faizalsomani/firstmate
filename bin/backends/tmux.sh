...
#
# Worktree acquisition (durably leasing the worktree from firstmate's own
# process, sending the pane a `cd` into it, and polling its cwd) is unchanged
# by this extraction: P1 scopes only the session provider, not the worktree
# provider, so fm-spawn.sh still drives that part inline with these same
