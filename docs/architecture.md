...
Whether a target project repo's `.no-mistakes/evidence/` commits ride along in the crew branch is opt-in per project, not a pipeline default; `AGENTS.md` owns the mechanism and the handling rule for a branch that has opted in.
The firstmate repo itself opts out: its `.no-mistakes/` directory is local state, stays gitignored, and is rejected by CI if tracked.
