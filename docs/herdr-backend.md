...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the two fixed spawn-time commands (the leased-worktree `cd`, the `GOTMPDIR` export) exactly where tmux used one `send-keys ... Enter` call. |
